// This design has replaced its joints by actual connections

outer_width = 40; // mm
outer_depth = 30; // mm
outer_height = 80; // mm
wall_thickness = 2; // mm
inner_width = outer_width - (2 * wall_thickness); // mm
inner_depth = outer_depth - (2 * wall_thickness); // mm
inner_height = outer_height - (2 * wall_thickness); // mm
n_teeth = 10;
...
// Creates the hollow cube
module create_hollow_cube() {
  difference() {
...
// The hollow cube with the top part removed. No teeth joint yet
module create_open_cube() {
  difference() {
    create_hollow_cube();
    translate([0, 0, outer_height - wall_thickness])
...
/*
module create_top_piece() {
  cube([outer_width,outer_depth,wall_thickness], center = true);
}


translate([0, 0, outer_height / 2 + spacing + spacing])
  create_top_piece();
